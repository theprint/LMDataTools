...
def generate_auto_perspectives(client, topic, count):
    ...
    for line in lines:
        ...
        if match:
            perspectives.append((match.group(1), match.group(2)))
...
def generate_qa_for_chunk(client, content, perspectives):
    ...
    for who, doing_what in perspectives:
        perspective = f"{who} who is {doing_what}"
...
def validate_qa_pair(question, answer):
    ...
    if not starts_with_capital(answer):
...
if __name__ == "__main__":
    ...
    try:
        ...
        final_file = os.path.join(OUTPUT_PATH, f"{DEFAULT_CONFIG['DATASET_NAME']}-{size_k}k.json")
        save_json(all_qa_data, final_file)
