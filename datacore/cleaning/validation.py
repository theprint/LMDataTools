...
def is_complete_answer(text: str) -> bool:
    ...
    if any(stripped.endswith(ending) for ending in valid_endings):
        return True
    # Also accept if the last non-empty line ends with a word character (e.g. a list
    # item or sentence that lacks terminal punctuation but is otherwise complete).
    last_line = stripped.rsplit('\n', 1)[-1].rstrip()
    return bool(re.search(r'\w$', last_line))
